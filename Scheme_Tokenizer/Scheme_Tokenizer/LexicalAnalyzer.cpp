...
// Default Constructor
LexicalAnalyzer::LexicalAnalyzer() : state(0), pos(0), curr(0), lineNum(0), errorCount(0) {
...
inline void LexicalAnalyzer::generateError(std::vector<Token>& result, const std::string& line, std::string& tokenstr) {
    ...
    while (pos < line.length()) {
        ...
        if ((curr == ' ') || (curr == 9) || (curr == ')') || (curr == '(')) {
            ...
            Token tok;
            tok.lexeme = SchemeLexemes::LEXICAL_ERROR;
            tok.value = tokenstr;
...
inline void LexicalAnalyzer::generateToken(std::vector<Token>& result, const std::string& line, std::string& tokenstr) {
    ...
    if (tok.lexeme == SchemeLexemes::COMMENT_T) {
        // Line comment
        // Ignore Line
        pos = line.length();
        return;
    }
    else if (tok.lexeme == SchemeLexemes::STRING_T) {
        // String Literal
        // Erase leading " mark
        // Skip trailing " mark
...
inline SchemeLexemes LexicalAnalyzer::terminalToLexeme() {
    SchemeLexemes lex = static_cast<SchemeLexemes>(state-STATE_OFFSET);
    return lex;
}
